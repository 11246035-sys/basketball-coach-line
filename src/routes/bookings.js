...
router.post('/', async (req, res) => {
  try {
    ...
    if (grade && !validGrades.includes(grade)) return res.status(400).json({ error: '無效的年級選項' });
    if (level && !validLevels.includes(level)) return res.status(400).json({ error: '無效的程度選項' });
    if (phone && !phoneRegex.test(phone)) return res.status(400).json({ error: '請輸入有效的電話號碼' });
    ...
    if (!dateRegex.test(date)) return res.status(400).json({ error: '日期格式錯誤' });
    const today = new Date().toISOString().split('T')[0];
    if (date < today) return res.status(400).json({ error: '不能預約過去的日期' });
    const { count: blacklistCount, error: blErr } = await supabase
      .from('blacklist').select('id', { count: 'exact', head: true }).eq('line_user_id', line_user_id);
    if (blErr) throw blErr;
    if (blacklistCount > 0) return res.status(403).json({ error: '無法完成預約，如有疑問請直接聯繫教練' });
    const { count: pendingCount, error: pendingErr } = await supabase
      .from('bookings').select('id', { count: 'exact', head: true })
      .eq('line_user_id', line_user_id).eq('status', 'pending');
    if (pendingErr) throw pendingErr;
    if (pendingCount >= 3) return res.status(400).json({ error: '您目前有 3 筆待確認的預約，請等教練確認後再預約新課程' });
    const { data, error } = await supabase.from('bookings')
      .insert([{ line_user_id, student_name, grade, level, phone, date, session, location, goal, notes, status: 'pending' }])
      .select().single();
    if (error) { console.error('[Bookings] 新增失敗:', error.message); return res.status(500).json({ error: '預約失敗，請稍後再試' }); }
    console.log('[Bookings] 新預約: ' + student_name);
  ...
  } catch (err) { console.error('[Bookings] 未預期錯誤:', err.message); res.status(500).json({ error: '系統錯誤' }); }
});
...
router.patch('/:id/cancel', async (req, res) => {
  try {
    ...
    if (!['pending','confirmed'].includes(booking.status)) return res.status(400).json({ error: '此預約無法取消' });
    ...
    if ((classTime - new Date()) / (1000*60*60) < 24) return res.status(400).json({ error: '距離上課不足 24 小時，如需取消請直接聯絡教練' });
    ...
    res.json({ success: true, booking: data });
  } catch (err) { console.error('[Bookings] 取消失敗:', err.message); res.status(500).json({ error: '取消失敗，請稍後再試' }); }
});
...
router.patch('/:id', requireAdmin, async (req, res) => {
  try {
    ...
    if (!validStatuses.includes(status)) return res.status(400).json({ error: '無效的狀態值' });
    ...
    if (status === 'confirmed' && booking.line_user_id)
      await pushMessage(booking.line_user_id, bookingConfirmFlex(booking)).catch(err => console.error('[Bookings]', err.message));
    if (status === 'rejected' && booking.line_user_id)
      await pushMessage(booking.line_user_id, textMessage('很抱歉，您的預約申請未能確認。\n\n學生：' + booking.student_name + '\n日期：' + booking.date + '\n\n如有疑問請直接聯繫教練。')).catch(err => console.error('[Bookings]', err.message));
    res.json({ success: true, booking: data });
...
router.get('/my/:lineUserId', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('bookings').select('*').eq('line_user_id', lineUserId).order('date', { ascending: false });
    if (error) throw error;
    res.json({ bookings: data });
  } catch (err) { res.status(500).json({ error: '查詢失敗' }); }
});
